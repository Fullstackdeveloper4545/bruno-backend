...
CREATE TABLE IF NOT EXISTS categories (
    ...
    name_es VARCHAR(150),
    image_url TEXT,
    show_complete_the_look BOOLEAN DEFAULT FALSE,
    complete_the_look_category_ids JSONB DEFAULT '[]'::jsonb,
    is_active BOOLEAN DEFAULT TRUE,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
async function ensureSchema() {
  ...
  await pool.query(`ALTER TABLE categories ADD COLUMN IF NOT EXISTS show_complete_the_look BOOLEAN DEFAULT FALSE`);
  await pool.query(`ALTER TABLE categories ADD COLUMN IF NOT EXISTS complete_the_look_category_ids JSONB DEFAULT '[]'::jsonb`);
