...
async function createCategory(req, res) {
  try {
    ...
    const resolvedCompleteLookCategories = parseStringArray(complete_the_look_category_ids);
    const result = await pool.query(
      ...
      [
        ...
        parent_id || null,
        image_url || null,
        parseBoolean(show_complete_the_look, false),
        JSON.stringify(resolvedCompleteLookCategories),
        is_active,
      ]
    );
    res.status(201).json(result.rows[0]);
...
async function updateCategory(req, res) {
  try {
    ...
    const resolvedCompleteLookCategories =
      complete_the_look_category_ids === undefined ? null : parseStringArray(complete_the_look_category_ids);
    const result = await pool.query(
      `UPDATE categories
       SET name_pt = COALESCE($1, name_pt),
           ...
           show_complete_the_look = COALESCE($6, show_complete_the_look),
           complete_the_look_category_ids = COALESCE($7::jsonb, complete_the_look_category_ids),
           is_active = COALESCE($8, is_active),
           updated_at = NOW()
       WHERE id = $9
       RETURNING *`,
      [
        ...
        parent_id,
        image_url,
        show_complete_the_look === undefined ? null : parseBoolean(show_complete_the_look),
        resolvedCompleteLookCategories == null ? null : JSON.stringify(resolvedCompleteLookCategories),
        is_active,
        id,
      ]
    );
