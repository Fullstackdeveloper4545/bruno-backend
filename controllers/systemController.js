...
async function getThemeSettings(req, res) {
  try {
    ...
    const public_home_hero_carousel_images = Array.isArray(effective.public_home_hero_carousel_images)
      ? effective.public_home_hero_carousel_images.map(img => ({
          url: typeof img?.url === 'string' ? img.url.trim() : '',
          title: typeof img?.title === 'string' ? img.title.trim() : '',
...
async function updateThemeSettings(req, res) {
  try {
    ...
    const public_home_hero_carousel_images = Array.isArray(payload.public_home_hero_carousel_images)
      ? payload.public_home_hero_carousel_images.map(img => ({
          url: typeof img?.url === 'string' ? img.url.trim() : '',
          title: typeof img?.title === 'string' ? img.title.trim() : '',
          alt: typeof img?.alt === 'string' ? img.alt.trim() : '',
        })).filter(img => img.url)
